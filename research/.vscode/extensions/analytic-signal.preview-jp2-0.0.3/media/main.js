...
(function () {
	...
	function getSettings() {
		const element = document.getElementById('preview-jp2-settings');
		if (element) {
	...
	function setImageSrc(url) {
		...
		req.onload = function (e) {
			if (req.status == 200) {
				image.src = getDataURL(url,req.response);
			}
		};
		req.send();
	}

	function getDataURL(src,response) {
		
		var extension = 'jp2';
		if (src.includes(".j2k?version")) {
			extension = 'j2k';
		}
		...
		var rgbImage = openjpeg(buffer, extension);
